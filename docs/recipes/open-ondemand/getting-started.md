...
- A **job management interface** for creating, templating, and monitoring Slurm jobs
- **Shell access** (a full terminal in your browser tab)
- **Interactive apps** (launch JupyterLab, RStudio, VS Code, MATLAB, Mathematica, and more with a few clicks)
- A **Module Explorer** for browsing available software without the command line
...
1. Open a browser and go to [{{ cluster.ondemand_url }}]({{ cluster.ondemand_url }}).

2. You'll see a **federated login page** listing participating institutions. Select **{{ institution.sso_dropdown_label }}** from the dropdown, then log in with your campus credentials (the same username and password you use for other {{ institution.short_name }} services).

3. After login, you'll land on the **OOD dashboard**.

!!! note "New accounts take a few minutes"
    If you just created your {{ cluster.name }} account, it may take up to 10 minutes before you can log in to OOD. If you see an authentication error immediately after account creation, wait a few minutes and try again.
...
- **Jobs** — Contains three tools:
    - **Job Composer** for creating and submitting batch jobs
    - **Templates** for saving and reusing job configurations
    - **Active Jobs** for monitoring running and queued jobs (based on `squeue`, so completed jobs disappear after about 5 minutes)
- **Shell** — Opens a terminal that connects you to {{ cluster.name }} exactly as SSH would, but running in your browser.
- **Interactive Apps** — Launch graphical applications on compute nodes: JupyterLab, RStudio, VS Code, MATLAB, Mathematica, Virtual Desktop (XFCE), and more. This menu also includes the **Module Explorer**, a searchable web interface for browsing available software (equivalent to `module spider` on the command line).

## Managing files

The **Files** menu opens a file browser that works much like a desktop file manager. From here you can:

- **Navigate** your home directory, work space, and scratch space using the path bar at the top
...
- **Open in Terminal** to jump from a directory straight into a shell session
...
The **Shell** menu → **{{ cluster.name }} Shell Access** opens a terminal in your browser tab. This is functionally identical to an SSH session:
...
!!! warning "Avoid `conda init` in the OOD shell"
    If you have a self-installed version of Anaconda or Miniconda and have run `conda init`, the OOD shell may crash on startup. Use the pre-installed Unity modules (`module load conda/latest`) instead, and avoid running `conda init`. If your shell is already broken, remove the conda initialization block from your `~/.bashrc` to fix it.

## What's next

- [**Interactive Apps in Open OnDemand**](apps.md) — launch Jupyter, RStudio, VS Code, and more through OOD without any SSH tunneling
