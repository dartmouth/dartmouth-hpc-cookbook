...
- SSH-based macros (`remote_cmd`, `system_stats`, `cluster_stats`) use SSH auth (key or GSSAPI, controlled by `build.ssh_auth` in `site.yml`) with `BatchMode=yes` and 5s/15s timeouts — they are designed to degrade gracefully.
